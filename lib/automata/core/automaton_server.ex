defmodule Automata.AutomatonServer do
  ...
  def start_link([node_sup, tree_config]) do
    GenServer.start_link(__MODULE__, [node_sup, tree_config], name: name(tree_config[:name]))
  end
  ...
  def init([node_sup, tree_config]) when is_pid(node_sup) do
    Process.flag(:trap_exit, true)
    monitors = :ets.new(:monitors, [:private])
    state = %State{node_sup: node_sup, monitors: monitors}

    init(tree_config, state)
  end
